/**
 ...
 @Name : layDate v1.1 Datetimepicker Plugin
 @Author: 贤心
 @Date: 2014-06-25
 @QQ群：176047195
...
// You can use the default setting without changing anything
var config =  {
    path: '', //path to laydate
    defSkin: 'default', //initialize the skin
    format: 'YYYY-MM-DD', //Datetime format
    min: '1900-01-01 00:00:00', //Minimal Date
    max: '2099-12-31 23:59:59', //Maximun Date
    isv: false
};
...
//Check whether the date is inside the effective range
...
//check the validation of hour, minute and second
...
//Check whether the date is legal or not
Dates.check = function(){
    ...
    if(value.replace(/\s/g, '') !== ''){
        if(!exp.test(value)){
            Dates.elem[as.elemv] = '';
            Dates.msg('The date is not valid, please reset it');
            return 1;
        } else if(isvoid[0]){
            Dates.elem[as.elemv] = '';
            Dates.msg('The date is outside the effective date, please reset it');
            return 1;
        } else {
...
//initialize the data for the panel
...
Dates.viewtb = (function(){
    var tr, view = [], weeks = [ 'Sun', 'Mon', 'Tue', 'Wed', 'Thur', 'Fri', 'Sat'];
...
Dates.events = function(){
    ...
    //Generate hour, minute and second info or warning info
    Dates.msg = function(i, title){
        var str = '<div class="laydte_hsmtex">'+ (title || 'Warning: ') +'<span>×</span></div>';
        if(typeof i === 'string'){
            str += '<p>'+ i +'</p>';
            Dates.shde(S('#'+as[0]));
